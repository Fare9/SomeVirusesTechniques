...
bool process_injector_t::inject_into_process()
{
	...
	{
		if (!allocate_new_space(this->process_handler, NULL, this->image_size_))
			return false;
	}

	if (!inject_pe_header(this->allocated_virtual_address_, this->process_handler))
		return false;

	if (!inject_sections_to_executable(this->allocated_virtual_address_, this->process_handler))
		return false;

	if (!load_imports(this->allocated_virtual_address_, this->process_handler))
		return false;

	if (!fix_relocs(this->allocated_virtual_address_, this->process_handler))
		return false;
...
HANDLE process_injector_t::create_thread_on_ep()
{
	uintptr_t address_to_thread = (uintptr_t)this->base_address_ + this->entry_point_;
	DWORD thread_id;

	HANDLE thread_handle = CreateRemoteThread(this->process_handler, NULL, 0, (LPTHREAD_START_ROUTINE)address_to_thread, NULL, NULL, &thread_id);
...
{
	return this->allocated_virtual_address_;
}
...
{
	allocated_virtual_address_ = VirtualAllocEx(
		hProcess,
		base_address,
		image_size,
	...
	if (allocated_virtual_address_ == NULL)
		return false;
	return true;
}
...
bool process_injector_t::load_imports(PVOID base_address, HANDLE hProcess)
{
	...
	while (descriptor[index].Name != 0)
	{
		...
		name_address = ((uintptr_t)this->process_buffer + (uintptr_t)this->rva_to_offset(this->process_buffer, descriptor[index].Name));
		...
		uintptr_t first_thunk_offset = (uintptr_t)this->process_buffer + (uintptr_t)rva_to_offset(this->process_buffer, descriptor[index].FirstThunk);
		uintptr_t origninal_first_thunk_offset;

		if (descriptor[index].OriginalFirstThunk != 0)
			origninal_first_thunk_offset = (uintptr_t)this->process_buffer + (uintptr_t)rva_to_offset(this->process_buffer, descriptor[index].OriginalFirstThunk);
		else
			origninal_first_thunk_offset = (uintptr_t)this->process_buffer + (uintptr_t)rva_to_offset(this->process_buffer, descriptor[index].FirstThunk);
...
bool process_injector_t::fix_relocs(PVOID base_address, HANDLE hProcess)
{
	IMAGE_DOS_HEADER* image_dos_header = (IMAGE_DOS_HEADER*)this->process_buffer;
	IMAGE_NT_HEADERS* image_nt_headers = (IMAGE_NT_HEADERS*)((uintptr_t)this->process_buffer + image_dos_header->e_lfanew);
	IMAGE_DATA_DIRECTORY* image_data_directory = &(image_nt_headers->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC]);

	DWORD delta = (uintptr_t)base_address - (uintptr_t)this->base_address_;

	uint64_t table_entry_offset = (uint64_t)this->process_buffer + this->rva_to_offset(this->process_buffer, image_data_directory->VirtualAddress);

	IMAGE_BASE_RELOCATION* table_entry = (IMAGE_BASE_RELOCATION*)table_entry_offset;
	
	// keep going until base relocation block consists of zero bytes
	while (table_entry->SizeOfBlock > 0)
	{
		uintptr_t page_address;
		DWORD n_relocs;
		uintptr_t reloc_record_offset;
		RELOC_RECORD* reloc_record;
		DWORD index;

		// calculate the address of the 4KB page to be fixed-up
		page_address = (uintptr_t)this->allocated_virtual_address_ + table_entry->VirtualAddress;

		// determine number of IMAGE_RELOC elements in the relocation block
		n_relocs = table_entry->SizeOfBlock;
		n_relocs -= sizeof(IMAGE_BASE_RELOCATION); // decrement the size of first struct
		n_relocs /= sizeof(RELOC_RECORD); // size divided by struct will be number of relocations
		
		// address of IMAGE_RELOC following IMAGE_BASE_RELOCATION
		reloc_record_offset = (uintptr_t) table_entry_offset + sizeof(IMAGE_BASE_RELOCATION);
		reloc_record = (RELOC_RECORD*)reloc_record_offset;

		for (index = 0; index < n_relocs; index++)
		{
			uintptr_t fix_up_address;
			DWORD fixup_type;
			uintptr_t value;
			SIZE_T read_size;
			// find fix-up address within 4KB byte (then add delta)
			fix_up_address = page_address + reloc_record[index].offset;
			fixup_type = reloc_record[index].type;

			if (!ReadProcessMemory(
				this->process_handler,
				(LPCVOID)fix_up_address,
				&value,
				sizeof(uintptr_t),
				&read_size
			))
			...
			if (fixup_type == IMAGE_REL_BASED_HIGH)
			{
				value += HIWORD(delta);
			}
			else if (fixup_type == IMAGE_REL_BASED_LOW)
			{
				value += LOWORD(delta);
			}
			else if (fixup_type == IMAGE_REL_BASED_HIGHLOW)
			{
				value += delta;
			}
		...
		// get the next entry in te relocation table
		table_entry_offset += table_entry->SizeOfBlock;
		table_entry = (IMAGE_BASE_RELOCATION*)table_entry_offset;
	}
